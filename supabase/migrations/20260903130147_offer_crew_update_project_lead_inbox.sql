-- Offer/crew-answer updates belong to the project lead inbox only.
-- created_by_user_id was the company owner, so owners saw every job's updates.
-- Also stamp offer metadata with recipient_status so accepted/declined UI matches crew answers.
...
SET metadata = metadata || jsonb_build_object('recipient_status', 'accepted')
WHERE matter_type = 'update'
  AND metadata ? 'accepted_at'
  AND NOT (metadata ? 'recipient_status');

UPDATE public.matters
SET metadata = metadata || jsonb_build_object('recipient_status', 'declined')
WHERE matter_type = 'update'
  AND metadata ? 'rejected_at'
  AND NOT (metadata ? 'recipient_status');

UPDATE public.matters m
SET created_by_user_id = j.project_lead_user_id
FROM public.jobs j
WHERE m.job_id = j.id
  AND m.matter_type = 'update'
  AND j.project_lead_user_id IS NOT NULL
  AND m.created_by_user_id IS DISTINCT FROM j.project_lead_user_id
  AND (
    m.metadata ? 'offer_id'
    OR m.metadata ? 'source_crew_invite_matter_id'
  );
...
WHERE mr.matter_id = m.id
  AND m.matter_type = 'update'
  AND j.project_lead_user_id IS NOT NULL
  AND mr.user_id IS DISTINCT FROM j.project_lead_user_id
  AND (
    m.metadata ? 'offer_id'
    OR m.metadata ? 'source_crew_invite_matter_id'
  );
