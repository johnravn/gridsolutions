-- job_copy returns { job_id, conflicts } so the app can toast without a second RLS-bound RPC.
-- Vehicle conflict listing uses the same booking window as the insert trigger (during / COALESCE).
-- Reservation start/end are shifted by the copy delta so custom windows stay on the new dates.
...
GRANT EXECUTE ON FUNCTION public.job_copy(uuid, timestamptz, timestamptz, text)
  TO authenticated, service_role;

COMMENT ON FUNCTION public.job_copy(uuid, timestamptz, timestamptz, text)
  IS 'Copies a job and returns { job_id, conflicts } where conflicts lists booking types that overlap';


-- Align vehicle conflict listing with the booking window used by the overlap trigger.
...
BEGIN
  ...
  SELECT
    ...
    a.eff_start AS start_1,
    a.eff_end AS end_1,
    b.eff_start AS start_2,
    b.eff_end AS end_2,
  ...
  FROM (
    SELECT
      ...
      COALESCE(rv.start_at, tp.start_at) AS eff_start,
      COALESCE(rv.end_at, tp.end_at) AS eff_end
    FROM public.reserved_vehicles rv
    JOIN public.time_periods tp ON tp.id = rv.time_period_id
    LEFT JOIN public.jobs j ON j.id = tp.job_id
    LEFT JOIN public.vehicles veh ON veh.id = rv.vehicle_id
    WHERE tp.company_id = p_company_id
      AND COALESCE(tp.deleted, false) = false
      AND COALESCE(veh.deleted, false) = false
      AND (p_from IS NULL OR COALESCE(rv.end_at, tp.end_at) >= p_from)
      AND (p_to IS NULL OR COALESCE(rv.start_at, tp.start_at) <= p_to)
  ) a
  JOIN (
    SELECT
      rv.vehicle_id,
...
COMMENT ON FUNCTION public.get_conflicts_vehicle(uuid, timestamptz, timestamptz)
  IS 'Returns overlapping vehicle bookings using the effective booking window (custom start/end or time period)';

-- Recompute stale during when the vehicle has no custom start/end.
UPDATE public.reserved_vehicles rv
SET during = tstzrange(tp.start_at, tp.end_at, '[)')
FROM public.time_periods tp
WHERE tp.id = rv.time_period_id
  AND rv.start_at IS NULL
  AND rv.end_at IS NULL
  AND rv.during IS DISTINCT FROM tstzrange(tp.start_at, tp.end_at, '[)');
