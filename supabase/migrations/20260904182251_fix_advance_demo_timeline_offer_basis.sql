-- Fix advance_demo_company_timeline after offer lines moved from offer_id → offer_basis_id.
-- The previous UPDATE joined offer_crew_items / offer_transport_items on oci.offer_id,
-- which no longer exists (see 20260704202526_add_offer_bases.sql), causing 42703 / HTTP 400.
...
BEGIN
  ...
  JOIN public.jobs j ON j.id = ob.job_id
  WHERE oci.offer_basis_id = ob.id
    AND ob.company_id = ANY (v_demo_company_ids)
    AND j.archived = false;
  ...
  FROM public.offer_bases ob
  JOIN public.jobs j ON j.id = ob.job_id
  WHERE oti.offer_basis_id = ob.id
    AND ob.company_id = ANY (v_demo_company_ids)
    AND j.archived = false;
...
COMMENT ON FUNCTION public.advance_demo_company_timeline(interval) IS
  'Advance demo company job/offer/time timelines by p_interval (default 7 days). Superuser or pg_cron only.';
