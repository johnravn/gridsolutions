-- Restore group_contents on public_offer_get equipment items.
-- Later pretty-offer / offer-basis rewrites of public_offer_get dropped this field,
-- so the public technical offer could no longer expand inventory item groups.
...
COMMENT ON FUNCTION public.public_offer_get(text) IS
  'Returns a non-draft offer by access_token for the public offer page (includes group_contents for expandable item groups).';
