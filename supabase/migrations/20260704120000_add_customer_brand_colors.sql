-- Customer brand colors (Radix accent + optional custom hex override)

ALTER TABLE public.customers
  ADD COLUMN IF NOT EXISTS accent_color TEXT,
  ADD COLUMN IF NOT EXISTS accent_color_custom TEXT;

ALTER TABLE public.customers
  DROP CONSTRAINT IF EXISTS customers_accent_color_check;

ALTER TABLE public.customers
  ADD CONSTRAINT customers_accent_color_check CHECK (
    accent_color IS NULL OR accent_color = ANY (ARRAY[
      'gray', 'gold', 'bronze', 'brown', 'yellow', 'amber', 'orange', 'tomato',
      'red', 'ruby', 'pink', 'plum', 'purple', 'violet', 'iris', 'indigo',
      'blue', 'cyan', 'teal', 'jade', 'green', 'grass', 'mint', 'lime', 'sky'
    ]::text[])
  );

ALTER TABLE public.customers
  DROP CONSTRAINT IF EXISTS customers_accent_color_custom_check;

ALTER TABLE public.customers
  ADD CONSTRAINT customers_accent_color_custom_check CHECK (
    accent_color_custom IS NULL OR accent_color_custom ~ '^#[0-9A-Fa-f]{6}$'
  );

COMMENT ON COLUMN public.customers.accent_color IS 'Radix UI accent color for customer branding in pretty offers.';
COMMENT ON COLUMN public.customers.accent_color_custom IS 'Optional hex accent override (#RRGGBB). Takes precedence over accent_color when set.';

-- Include customer brand colors in public_offer_get
