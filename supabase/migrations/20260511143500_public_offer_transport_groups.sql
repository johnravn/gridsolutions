-- Extend public_offer_get to return transport groups (with ordered items) for customer presentation.
-- Keeps legacy transport_items as a flattened ordered list for backwards compatibility.
...
BEGIN
  ...
  -- Transport groups + items (attach minimal vehicle info when present)
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        'id', tg.id,
        'offer_id', tg.offer_id,
        ...
        'items',
          COALESCE((
            ...
            WHERE ti.transport_group_id = tg.id
          ), '[]'::jsonb)
      )
  ...
  -- Flattened transport items for older clients (ordered by group then item)
  SELECT COALESCE(
    jsonb_agg(
      to_jsonb(ti) ||
      jsonb_build_object(
      ...
      ORDER BY tg.sort_order, ti.sort_order
    ),
    '[]'::jsonb
  )
