...
export type SendCrewPositionInviteEmailResponse =
  | { ok: true }
  | { error: string; details?: string }
 ...
 */
export async function invokeEmailEdgeFunction<
...
>(
  ...
  | { ok: true; data: T }
  | { ok: false; failure: EmailEdgeInvokeFailure; data: T | null }
> {
  ...
  if (error) {
    ...
    const hint404 =
      status === 404 ? hintForFunctionsHttp404(functionName) : undefined
    return {
      ok: false,
      data: data ?? null,
      failure: {
...
export async function sendOfferByEmail(params: {
  ...
  | { ok: false; failure: EmailEdgeInvokeFailure }
> {
  const res = await invokeEmailEdgeFunction<SendOfferEmailResponse>(
    'send-offer-email',
    {
  ...
  const messageId =
    (res.data as { message_id?: string | null }).message_id ?? null
  return { ok: true, messageId }
}

export async function sendWelcomeEmailForPendingInvite(
  pendingInviteId: string,
): Promise<{ ok: true } | { ok: false; failure: EmailEdgeInvokeFailure }> {
  const res = await invokeEmailEdgeFunction<SendWelcomeEmailResponse>(
    'send-welcome-email',
    {
...
export async function sendNotificationEmailNow(params: {
  ...
  const res = await invokeEmailEdgeFunction<SendNotificationEmailResponse>(
    'send-notification-email',
    {
...
export async function sendMatterEmailTest(params: {
  ...
  | { ok: false; failure: EmailEdgeInvokeFailure }
> {
  const res = await invokeEmailEdgeFunction<SendTestEmailResponse>(
    'send-test-email',
    {
...
export async function sendCrewPositionInviteEmail(params: {
  ...
  const res =
    await invokeEmailEdgeFunction<SendCrewPositionInviteEmailResponse>(
      'send-crew-position-invite-email',
      {
...
export function fireAndForgetCrewPositionInviteEmail(
  ...
  void sendCrewPositionInviteEmail({ reservedCrewId, invitationMessage }).then(
    (r) => {
      if (!r.ok && import.meta.env.DEV) {
        console.warn(
          '[email] send-crew-position-invite-email failed',
          r.failure,
        )
      }
    },
