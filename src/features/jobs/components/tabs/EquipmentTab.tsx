    ...
    queryFn: async () => {
      ...
      const { data: items, error } = await supabase
        ...
        .select(
          ...
          external_status, external_note, forced, subcontractor_id,
          item:item_id (
...
function InternalEquipmentTable({
  ...
  rows: Array<any>
  groupItemsByGroupId: Map<string, Array<{ item_id: string; quantity: number }>>
  jobId: string
  canBook: boolean
  ...
  const getRowCategory = (row: any, sourceGroup?: any) => {
  ...
  const categorizedEntries = React.useMemo(() => {
    ...
    for (const groupRows of groupMap.values()) {
      ...
      const categoryName = getRowCategory(firstRow, sourceGroup)
    ...
    for (const row of directRows) {
      const categoryName = getRowCategory(row)
      const entry = categoryMap.get(categoryName) ?? {
        groupEntries: [],
        directRows: [],
      }
    ...
    const entries = Array.from(categoryMap.entries()).map(
      ([categoryName, entry]) => ({
        categoryName,
        groupEntries: entry.groupEntries,
        directRows: entry.directRows,
      }),
    )
  ...
  React.useEffect(() => {
    setExpandedCategories((prev) => {
      ...
      for (const entry of categorizedEntries) {
        if (!next.has(entry.categoryName)) {
          next.add(entry.categoryName)
          changed = true
        }
      }
  ...
  const handleSaveQty = async (rowId: string, newQty: number) => {
    ...
    try {
      ...
      p.success('Saved', 'Quantity updated')
  ...
  const handleDeleteSelected = async () => {
    ...
    const p = progress('Deleting bookings…')
    try {
                  ...
                  >
                    <Flex align="center" justify="between">
                      <Flex align="center" gap="3">
                        ...
                        <Text weight="medium">{category.categoryName}</Text>
...
function ExternalEquipmentTable({
  ...
  const handleUpdateOwnerItems = async (
    ...
    const p = progress('Updating bookings…')
    try {
      ...
      p.success('Updated', 'All items for this owner updated')
      void qc.invalidateQueries({ queryKey: ['jobs.equipment', jobId] })
    } catch (e: any) {
      p.error('Failed to update', e?.message || 'Please try again.')
    }
  ...
  const handleDeleteOwner = async () => {
    ...
    try {
      ...
      p.success('Deleted', `Removed booking for ${deleteOwnerOpen.ownerName}`)
  ...
  const handleUpdateItemStatus = async (
    ...
    try {
      ...
      p.success('Updated', 'Item status updated')
      void qc.invalidateQueries({ queryKey: ['jobs.equipment', jobId] })
    } catch (e: any) {
      p.error('Failed to update', e?.message || 'Please try again.')
    }
  ...
  const handleRecoupleRow = (rowId: string) => {
    setDecoupledRows((prev) => {
      const next = new Set(prev)
      next.delete(rowId)
      return next
