...
export function CrewItemsPanel({
  items,
  onItemsChange,
  ...
  >({})
  const [hoursPerDayDrafts, setHoursPerDayDrafts] = React.useState<
    Record<string, string>
  >({})
  ...
  const addItem = () => {
    ...
    const hoursPerDay = resolveHourlyHoursPerDay(startIso, endIso)
  ...
  const normalizeCrewItem = (
    item: LocalCrewItem,
    options?: { recomputeHoursFromDates?: boolean },
  ): LocalCrewItem => {
    if (item.billing_type === 'hourly') {
      const hoursPerDay = options?.recomputeHoursFromDates
        ? resolveHourlyHoursPerDay(item.start_date, item.end_date)
      ...
      return {
        ...
        daily_rate: normalizedHourly * hoursPerDay,
      }
    }
  ...
  const updateItem = (itemId: string, updates: Partial<LocalCrewItem>) => {
    ...
    const hoursExplicit = updates.hours_per_day !== undefined
    onItemsChange(
      items.map((item) =>
        item.id === itemId
          ? normalizeCrewItem(
              { ...item, ...updates },
              {
                recomputeHoursFromDates: datesChanged && !hoursExplicit,
              },
            )
          : item,
      ),
                                          ...
                                          <>
                                            ...
                                            <Box style={{ minWidth: 120 }}>
                                              ...
                                              <TextField.Root
                                                ...
                                                value={
                                                  hoursPerDayDrafts[item.id] ??
                                                  (formattedHoursPerDay != null
                                                    ? String(
                                                        formattedHoursPerDay,
                                                      )
                                                    : '')
                                                }
                                                onChange={(e) => {
